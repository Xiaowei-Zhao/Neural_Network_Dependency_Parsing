...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(stack) == 0:
            ...
            word = word + [stack[-1], None, None]
        elif len(stack) == 2:
            word = word + [stack[-1], stack[-2], None]
        else:
            word = word + [stack[-1], stack[-2], stack[-3]]

        if len(buffer) == 0:
            word = word + [None, None, None]
        elif len(buffer) == 1:
            word = word + [buffer[-1], None, None]
        elif len(buffer) == 2:
            word = word + [buffer[-1], buffer[-2], None]
        else:
            word = word + [buffer[-1], buffer[-2], buffer[-3]]
        ...
        for i in word:
            if i is None:
                ...
                word2index = self.word_vocab.get(words[i].lower(), None)
                if word2index is None:
                    input_re.append(2)
                else:
                    input_re.append(word2index)

        return np.array(input_re)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        output_res = np.zeros(91)
        output_label = self.output_labels
        #print(output_label)
        i = output_label.get(output_pair, None)
        if i is not None:
            output_res[i] = 1
        return output_res
...
if __name__ == "__main__":
    ...
    #              #print(state)
    #              #print(output_pair)
    #              #rep = extractor.get_input_representation(words, pos, state)
    #              #print(rep)
    #              #outputs=extractor.get_output_representation(output_pair)
    #              #print(outputs)

    # file = './data/train.conll'
    # with open(file,'r') as in_file:
    #     extractor = FeatureExtractor(word_vocab_f, pos_vocab_f)
    #     print("Starting feature extraction... (each . represents 100 sentences)")
    #     inputs, outputs = get_training_matrices(extractor,in_file)
